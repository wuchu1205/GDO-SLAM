...
class Metrics(object):
    ...
    def compute_metrics(self,):
        ...
        acc_cls = confusion.diag() / confusion.sum(dim=1)
        print("acc: ", acc_cls)
        # iou and fw miou
...
class MscEvalV0(object):
    ...
    def __call__(self, net, dl):
        ...
        for i, (imgs, label) in diter:
            ...
            # out = preds.squeeze().detach().cpu().numpy()
            # out = out * 255
            # pred_name = "_".join(re.split("\.|_", image_id[0].split("/")[-1])[:-1]) + "_predicted_mask.png"
        average_inference_time = np.mean(inferences_times)
...
def groundseg_eval_model(cfg, net):
    net.eval()
